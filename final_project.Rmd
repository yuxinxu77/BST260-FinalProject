...
library(pROC)
# train_regression <- cbind(X_train_pca, y_train) %>% rename(diabetes = y_train)
train_regression <- cbind(X_train, y_train) %>% rename(diabetes = y_train)
# glm_fit <- glm(diabetes ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data = train_regression, family=binomial)
# glm_fit <- glm(diabetes ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = train_regression, family=binomial)
...
# table(glm_predict, y_test)
# mean(glm_predict == y_test)
confusionMatrix(as.factor(glm_predict), as.factor(y_test))
...
tr <- trainControl(method = "cv", number = 10)
for (mtry in c(2,5,8)){
  for (nodesize in c(1,5,9)){
      print(paste("mtry =" , mtry , ",nodesize =" , nodesize , ",maxnodes" , maxnodes))
rf_classifier = randomForest(x = X_train_balanced_pca, y = y_train_balanced, xtest = X_test_balanced_pca, ytest=y_test, ntree=100, importance=TRUE, keep.forest=TRUE, trControl = tr, maxnodes = 10)
#rf_classifier
...
##SVM
```{r}
# SVM PCA, balanced: sensitivity 0.81, specificity 0.55, roc 0.76 Normal-Good

library(e1071)

svm <- svm(x = X_train_balanced_pca, y = y_train_balanced, scale = TRUE, kernel = 'poly', cross = 10, probability = TRUE)

pre_svm <- predict(svm, X_test_balanced_pca, probability = TRUE)
# print(pre_svm)
...
pre_svm <- ifelse(pre_svm > 0.35, 1, 0)
confusionMatrix(as.factor(pre_svm), y_test, positive = "1")

```

```{r}
# SVM PCA, not balanced: sensitivity 0.98, specificity 0.14, roc 0.72 Normal

svm <- svm(x = X_train_pca, y = y_train, scale = TRUE, kernel = 'poly', cross = 10, probability = TRUE)

pre_svm <- predict(svm, X_test_pca, probability = TRUE)
# print(pre_svm)
...
pre_svm <- ifelse(pre_svm > 0.1, 1, 0)
confusionMatrix(as.factor(pre_svm), y_test, positive = "1")
```
```{r}
...
```{r}
# SVM not PCA, not balanced: sensitivity 0.85, specificity 0.51, roc 0.77 Good 

svm <- svm(x = X_train, y = y_train, scale = TRUE, kernel = 'linear', cross = 10, probability = TRUE)
...
pre_svm <- ifelse(pre_svm > 0.15, 1, 0)
confusionMatrix(as.factor(pre_svm), y_test, positive = "1")
...
pre_knn <- knn(X_train_balanced, X_test , cl = y_train_balanced, k = 2, l = 0, prob = TRUE, use.all = TRUE)

pre_knn <- ifelse(pre_knn == 1, attr(pre_knn, "prob"), 1 - attr(pre_knn, "prob"))
plot_roc(as.numeric(pre_knn))

pre_knn <- ifelse(pre_knn > 0.25, 1, 0)
confusionMatrix(as.factor(pre_knn), y_test, positive = "1")
```
```{r}
# KNN pca, not balanced: sensitivity 0.21, specificity 0.86, roc 0.54 Very bad

pre_knn <- knn(X_train_pca, X_test_pca , cl = y_train, k = 2, l = 0, prob = TRUE, use.all = TRUE)

pre_knn <- ifelse(pre_knn == 1, attr(pre_knn, "prob"), 1 - attr(pre_knn, "prob"))
plot_roc(as.numeric(pre_knn))

pre_knn <- ifelse(pre_knn > 0.1, 1, 0)
confusionMatrix(as.factor(pre_knn), y_test, positive = "1")
...
pre_knn <- knn(X_train, X_test, cl = y_train, k = 2, l = 0, prob = TRUE, use.all = TRUE)
