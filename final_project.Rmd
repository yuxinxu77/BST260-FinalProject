...
library(ROSE)
data_balanced <- ovun.sample(y ~ ., data = train, method = "over",N = 5108)$data
table(data_balanced$y)
```
```{r}
# Perform training:
rf_classifier = randomForest(y~., data = data_balanced, xtest = X_test_pca, ytest=y_test, ntree=500, mtry=2, importance=TRUE, keep.forest=TRUE)
pre = predict(rf_classifier, X_test_pca)
pred = prediction(as.numeric(pre), as.numeric(y_test))
library(ROCR)
