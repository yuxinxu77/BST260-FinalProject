...
#rf_classifier
pre_prob = predict(rf_classifier, X_test_balanced_pca, type = "prob")[,1]
pre = ifelse(pre_prob >= 0.5, 0, 1)
confusionMatrix(as.factor(pre), as.factor(y_test))
plot_roc(pre_prob)
```
...
pred <- pre[,,1][,1]
y_pred <- ifelse(as.numeric(pred)>0.6,0,1)
confusionMatrix(as.factor(y_pred), as.factor(y_test))
plot_roc(as.numeric(pred))
```
```{r}
